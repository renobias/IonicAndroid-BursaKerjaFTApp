...
export class NotifCompanyPage {
  ...
  constructor(public Common:Common, public alertController:AlertController, public navCtrl: NavController, public navParams: NavParams,public authService:AuthServiceProvider) {
    ...
    this.authService.postData(this.userPostData,"daftarPenawaran").then(
      result => {
        ...
        if (this.responseData.profileUserData) {
          ...
          localStorage.setItem('daftarPenawaran',JSON.stringify(this.responseData));
  ...
  berhentiPenawaran(index:any){
    ...
    console.log(dataPenawaran.profileUserData[index].id_penawaran);
       this.berhentipenawaranPostData.user_id=this.userDetails.user_id;
        ...
        this.berhentipenawaranPostData.id_penawaran=dataPenawaran.profileUserData[index].id_penawaran;
        console.log(this.berhentipenawaranPostData.id_penawaran);

        this.authService.postData(this.berhentipenawaranPostData,"berhentiPenawaran").then(
          result => {
            this.responseDataStop = result;
              console.log(this.responseDataStop);
              const alert = this.alertController.create({
                title: 'Berhasil',
                subTitle: 'Pencari kerja tersebut telah diberhentikan dari masa penawaran!',
                buttons: ['OK']
              });
              alert.present();
